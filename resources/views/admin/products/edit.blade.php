        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex gap-4 mb-4">
                <div class="w-1/3">
                    ...
                    <input type="number" name="price" id="price" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('price', $product->price) }}" step="0.01" min="0" required>
                </div>
                <div class="w-1/3">
                    <label for="stock" class="block text-gray-700 font-bold mb-2">Stock Quantity</label>
                    <input type="number" name="stock" id="stock" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('stock', $product->stock) }}" min="0" required>
                </div>
                <div class="w-1/3">
                    ...
                    <select name="rating" id="rating" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="1" {{ old('rating', $product->rating) == '1' ? 'selected' : '' }}>1 Star</option>
                        <option value="2" {{ old('rating', $product->rating) == '2' ? 'selected' : '' }}>2 Stars</option>
                        <option value="3" {{ old('rating', $product->rating) == '3' ? 'selected' : '' }}>3 Stars</option>
                        <option value="4" {{ old('rating', $product->rating) == '4' ? 'selected' : '' }}>4 Stars</option>
                        <option value="5" {{ old('rating', $product->rating) == '5' ? 'selected' : '' }}>5 Stars</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                @if($product->image)
                    <div class="mb-3">
                        ...
                        <img src="{{ asset($product->image) }}" alt="Current Product Image" class="h-32 object-contain border rounded p-2 bg-gray-50">
                    </div>
                @endif

                <input type="file" name="image" id="image" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" accept="image/*">
                <p class="text-sm text-gray-500 mt-1">Leave blank to keep current image. Accepted formats: JPEG, PNG, JPG, GIF. Max size: 2MB.</p>
            </div>
